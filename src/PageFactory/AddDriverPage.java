...
/*
 * author: anjana.iv
 * Purpose : Add and Edit features of Driver Object Locators and Methods
 */
...
public class AddDriverPage {
	...
	public void setDrivingLicNo(String DrivingLicNo){
		jse.executeScript("arguments[0].scrollIntoView(true);",txt_drivingLicNo);
		txt_drivingLicNo.clear();
		txt_drivingLicNo.sendKeys(DrivingLicNo);
		Reporter.log("Driving Licence No "+DrivingLicNo+"<br>");
	}
	public boolean fill_AddDriver_form(Object[] FormData,String sForm) throws Exception{
		try {
		...
		if(FormData[9].toString()!= "") {
			this.setDrivingLicNo(FormData[9].toString());}
		...
		Reporter.log("Add Driver Form is filled with above details<br>");
		if(FormData[17].toString().equalsIgnoreCase("TRUE")) {
			...
			Reporter.log("Encountered issues while filling up the Add Edit Driver form"+e.getMessage());
			return false;
		}
		return true;
		
	}
	public boolean fill_EditDriver_form(Object[] FormData,String sForm) throws Exception{
		try {
		CommonPageObjects cpo = new CommonPageObjects(driver);
		if(FormData[1].toString()!= "") {
		...
		if(FormData[10].toString()!= "") {
			this.setDrivingLicNo(FormData[10].toString());}
		if(FormData[11].toString()!= "") {
		...
		if(FormData[18].toString().equalsIgnoreCase("TRUE")) {
			...
			Reporter.log("Encountered issues while filling up the Edit Driver form"+e.getMessage());
			return false;
		}
		return true;
